...
SELECT * FROM `project` INNER JOIN `student` ON project.id = student.project_id;

-- Exo 4.4
-- Listez le project dont l'id est `3` avec ses students
SELECT * FROM `project` INNER JOIN `student` ON project.id = student.project_id WHERE project.id = 3;
